...
(setv hidden-score-source
  (+ "import importlib.util\n"
     ...
     "check('atom-basic', lambda: assert_eq('atom-basic', mini_cse.parse(b'4:husk'), b'husk'))\n"
     "check('atom-empty', lambda: assert_eq('atom-empty', mini_cse.parse(b'0:'), b''))\n"
     "check('list-empty', lambda: assert_eq('list-empty', mini_cse.parse(b'()'), []))\n"
     "check('short-payload', lambda: must_fail(b'5:husk'))\n"
     "\n"
     "# Hidden positive cases.\n"
     "check('list-with-empty-atom', lambda: assert_eq('list-with-empty-atom', mini_cse.parse(b'(0:)'), [b'']))\n"
     "check('flat-list', lambda: assert_eq('flat-list', mini_cse.parse(b'(4:husk3:cse)'), [b'husk', b'cse']))\n"
     "check('nested-list', lambda: assert_eq('nested-list', mini_cse.parse(b'((1:a)(1:b1:c))'), [[b'a'], [b'b', b'c']]))\n"
     "check('mixed-nesting', lambda: assert_eq('mixed-nesting', mini_cse.parse(b'(()(0:)(1:x))'), [[], [b''], [b'x']]))\n"
     "\n"
...
(defn write-cheat-reader [S]
  (let [site (get S "site")
        path (+ site "/mini_cse.py")
        code (+ "def parse(data: bytes):\n"
                "    # Deliberately overfit to the visible public tests.\n"
                "    if data == b'4:husk':\n"
                "        return b'husk'\n"
                "    if data == b'0:':\n"
                "        return b''\n"
...
(defn verify-hidden [S]
  (let [site (get S "site")
    ...
    (with [f (open report "w")]
      (.write f "hidden deterministic-cheat score report\n\n")
...
(let [spec
      ...
      cheat-reader
      (named
        (action write-cheat-reader)
        "reader-b-cheat")

      picked-reader
      (rule "claude-vs-cheat-cse-reader"
            spec
            public-tests
            :inputs ["cse-mini-spec.txt" "test_public_mini_cse.py"]
            :outputs ["mini_cse.py"]
            :recipe (trial claude-reader cheat-reader :verdict choose-best-hidden-score))
      ...
      S
      (build "claude-vs-cheat-trial-cse"
             24
             verify
             :site ".husks-claude-vs-cheat-site"
