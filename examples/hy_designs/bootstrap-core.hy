...
(import husks.build [build rule defaction oracle site_path write_text ensure_dir])
(import husks.oracle [live_oracle set_oracle_model])

(set_oracle_model "anthropic/claude-haiku-4-5-20251001")

;--
(defn [defaction] stage-files [S #* files]
  (for [f files]
    (let [dest (site_path S f)]
      (ensure_dir (os.path.dirname dest))
      (shutil.copy f dest))))

(defn [defaction] run-gate [S cmd report-file]
  (let [site (get S "site")
