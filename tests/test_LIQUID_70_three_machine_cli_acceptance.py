...
def test_three_machine_cli_acceptance_stub():
    ...
    try:
        ...
        # ──────────────────────────────────────────────────────────
        # Beta Gate C/F/G: Compare sites (unified compare with 3+ sites
        # reads .traces/report.json and runs three-machine proof)
        # ──────────────────────────────────────────────────────────
        ...
        assert compare_result.returncode == 0, (
            f"compare should validate three-machine proof\n"
        ...
        assert compare["equivalent"] is True, (
            ...
            f"Proof: {compare.get('proof', {})}"
        )

        # Verify proof section is present and checks passed
        proof = compare.get("proof", {})
        if proof:
            checks = proof["checks"]
