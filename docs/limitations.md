...
## 11. ~~Non-shell recipe identity depends on host-language symbols~~ (resolved)

Resolved in CSE v2 (§E5).  Action recipes now seal a behavior-based
digest (source code hash) instead of `__qualname__`.  Built-in cond
predicates seal the full spec string (`file-exists:<path>`, etc.).
Renaming a function no longer changes the seal.  Different predicate
arguments now produce different seals.

Shell actions remain unchanged — the command string is the sole identity.
